...
from .utils import sanitization
...
def malware_detection(request):
        clf = joblib.load('static/Classifier/classifier.pkl')
        features = pickle.loads(open(os.path.join('static/Classifier/features.pkl'),'rb').read())
        #extracting features from the PE file mentioned in the argument 
        str2=str(id)
        data = extract_infos('static/msinfo32.exe')
        ...
        strr=str(['malicious','non-malicious'][res])
        context={
            "filename":"file1.exe",
            "results":strr
        }
        return render(request,'malware_detection.html',context)
...
def detect_malware(request):
    if request.method == 'POST':
        file = request.FILES['file']  # Assuming the file is sent as a POST file upload
        clf = joblib.load('static/Classifier/classifier.pkl')
        features = pickle.loads(open(os.path.join('static/Classifier/features.pkl'), 'rb').read())
        # Extracting features from the uploaded file
        data = extract_infos('static/'+file.name)  
        pe_features = list(map(lambda x:data[x], features))
        print("Features used for classification: ", pe_features)
...
def url_detection(request):
    ...
    #Loading the model
    file = "static/Classifier/pickel_model.pkl"
    with open(file, 'rb') as f1:  
        lgr = pickle.load(f1)
    f1.close()
    file = "static/Classifier/pickel_vector.pkl"
    ...
    context={
        "str1":predict[0],
        "str2": "If you feel that this prediction is wrong, or if you are not so sure about this output\nyou can contact us at patilsaurabh7777777.com we'll check the URL and update the machine\naccodingly. Thank you."
    }
    return render(request,"url_detection.html",context)
...
def password_generator(request):
    ...
    if fl == 1:
         return render(request, 'password.html', {'password':'0000'})
    else:
        password = ''
